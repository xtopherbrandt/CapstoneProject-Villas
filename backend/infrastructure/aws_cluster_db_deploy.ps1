...
Write-Output "Create cluster"
    eksctl create cluster --name villas-cluster --nodes=4 --version=1.27 --instance-types=t2.micro --region=us-east-2 --with-oidc
...
Write-Output "    provision database:"
    ...
    #also note that the user running this command needs rds:DescribeDB* permissions to allow it to create the instance.
    #these have been added through an inline policy call rds-describe
    kubectl apply -f .\rds-postgres.yaml

    kubectl describe dbinstance -n ${APP_NAMESPACE} ${RDS_DB_INSTANCE_NAME}
Write-Output "Wait for the DB Instance to be Availabile. Run the following to watch the status:"
Write-Output " (kubectl describe dbinstance -n villas-deploy villas-rds) -match 'Db Instance Status: (?<status>\w*)'"
Write-Output "When the instance is available run aws_villas_service_deploy.ps1"
